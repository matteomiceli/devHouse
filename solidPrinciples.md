...
For the most part we didn't violate the **Open Close Principle**. The Open Closed Principle meaning classes that we created and used should not be modified but rather extended and used from there. Before viewing the SOLID videos, Octavio modified the IPostService interface to include new functions like deletePost() and likePost().
